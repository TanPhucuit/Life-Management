...
function makeMockData(topicId: string, planetCount: number) {
  ...
  for (let index = 0; index < planetCount; index += 1) {
    ...
    planets.push({
      ...
      status: completion >= 1 ? 'completed' : completion > 0 ? 'in_progress' : 'not_completed',
    ...
    const nodes: TreeTaskInput[] = [{ id, parentId: null, title: `Project ${index + 1}`, done: completion >= 1, isLeaf: false }];
    const branches = 2 + (index % 4);
    for (let b = 0; b < branches; b += 1) {
      const branchId = `${id}-b${b}`;
      nodes.push({ id: branchId, parentId: id, title: `Branch ${b + 1}`, done: b % 3 === 0, isLeaf: false });
      const leaves = 2 + ((index + b) % 3);
      for (let l = 0; l < leaves; l += 1) {
        const leafId = `${branchId}-l${l}`;
        nodes.push({ id: leafId, parentId: branchId, title: `Task ${b + 1}.${l + 1}`, done: (l + b) % 2 === 0, isLeaf: l % 3 !== 0 });
        if (l % 3 === 0) {
          for (let g = 0; g < 2; g += 1) {
            nodes.push({ id: `${leafId}-g${g}`, parentId: leafId, title: `Step ${g + 1}`, done: g === 0, isLeaf: true });
          }
        }
      }
    }
